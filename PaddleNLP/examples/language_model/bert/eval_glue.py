...
def evaluate(model, metric, data_loader):
    ...
    if isinstance(metric, AccuracyAndF1):
        logger.info(
            "eval acc: %s, precision: %s, recall: %s, f1: %s, acc and f1: %s." %
            (res[0], res[1], res[2], res[3], res[4]))
    elif isinstance(metric, Mcc):
        logger.info("loss: %f, mcc: %s." % (res[0]))
    elif isinstance(metric, PearsonAndSpearman):
        logger.info("eval pearson: %s, spearman: %s, pearson and spearman: %s."
                    % (res[0], res[1], res[2]))
    else:
        logger.info("eval acc: %s." % (res))
    model.train()
...
def do_eval(args):
    ...
    tokenizer = tokenizer_class.from_pretrained("./best_finetune_model")
...
if __name__ == "__main__":
    ...
    if args.n_cards > 1 and args.select_device == "gpu":
        paddle.distributed.spawn(do_eval, args=(args, ), nprocs=args.n_cards)
    else:
        do_eval(args)
